        ...
        if ($.isArray(condition))
           ...
           if (condition.rules != null){ 
            for (var i = 0; i < condition.rules.length; i++){
            	if (condition.rules[i].type == "number"){
            		condition.rules[i].value = Number(condition.rules[i].value)
            	}
            }
           }
...
(function ($)
{
    ...
    $.ligerDefaults.GridString = {
        ...
        pageStatMessage: 'from {from} to {to}, total {total} items, pagesize：{pagesize}',
        ...
        */
        ...
        showFilter: function ()
        {
            ...
            return g.winfilter = $.ligerDialog.open({
                ...
                target: filtercontainer, isResize: true, top: 50,title: "Filter",
                buttons: [
