...
import reports.RunIRTaccountDetail;;

/**
 * Servlet implementation class ReportAccountDetailServlet
 * 查询account detail by company code
 */
public class ReportAccountDetailServlet extends HttpServlet {
	 ...
	 */
	public ReportAccountDetailServlet() {
		super();
		...
		while ((jsonRrequest = reader.readLine()) != null) {
			...
			try {
				...
				for (int i = 0; i < structure.size(); i++) {
					JSONArray reportTemp = RunIRTaccountDetail.startBalance(companies, currency, Fiscal_Year,
							Posting_period, structure.getString(i));
